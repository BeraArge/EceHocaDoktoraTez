...
namespace MSC.Extentions.Filters
{
    public class AuthorizeActionFilter : IAuthorizationFilter
    {
        ...
        public AuthorizeActionFilter(string controller = "", string action = "", bool loginAccess = true, IMemoryService memoryService = null)
        {
            ...
            try
            {
                ...
                if (this.controller != "" && this.action != "")
                {
                    ...
                    if (loginAccess)
                    {
                        ...
                        if (auth_id_key == null || auth_id_key == "")
                        {
                            context.Result = new RedirectResult("/auth/login?returnURL=" + controller + "/" + action + "?" + querystring);
                            context.HttpContext.Response.Redirect("/auth/login?returnURL=" + controller + "/" + action + "?" + querystring);
                        }
                        ...
                        var user_permissions_str = _memoryService.Get<string>(decauthid.Split('-')[0]);
                        if (string.IsNullOrEmpty(user_permissions_str))
                        {
                            var message = "";
                            context.HttpContext.Response.Cookies.Delete(decauthid);
