...
namespace BusinessLogicLayer.Helpers.SmsHelper
{
    public class SmsService : IOneTimePasswordService
    {
        ...
        public async Task<SendOtpResponse> SendOtp(string message, List<string> to)
        {
            var organikHaberlesme = _configuration.GetSection(nameof(OrganikHaberlesmeModel)).Get<OrganikHaberlesmeModel>();
            if (to.Count <= 0)
            {
                var res = new SendOtpResponse()
                {
                    Data = "Lütfen sms gönderilecek telefon numarasının girişini yapınız.",
                    Result = false
                };
                return res;
            }
            ...
            data = data.Replace("\"message\":\"\"", $"\"message\":\"{upperMessage}\"");
